...
def dualClass(N,clf,X,y,featureNumber):
    ...
    f2=np.array([30, 63, 64, 65, 99, 100, 240, 274, 275, 448, 449, 450, 485]) 
...
def ghostFeatures(rawData, indVal, chanNum, fs, lowcut, highcut, pcti):
    ...
    i2=int(i1+fs)
    can1=int(chanNum)
    ses1=np.squeeze(rawData[i1:i2,:])
    singChan=ses1[0::,can1]
    w1=[0,fs]
    w2=[int(fs+1),int(2*fs)]
...
def ghostFeaturesOld(rawData, indVal, chanNum, fs, lowcut, highcut, pcti, windows):
